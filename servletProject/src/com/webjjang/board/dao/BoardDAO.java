...
public class BoardDAO {
	...
	public List<BoardReplyVO> replyList(Long no, PageObject pageObject) throws Exception{
		...
		try {
			...
			e.printStackTrace();
			// 사용자를 위한 오류 처리
			throw new Exception("댓글 리스트 실행 중 DB 처리 오류");
		} finally {
			// 7. 닫기
			DBInfo.close(con, pstmt, rs);
		}
	...
	public long getReplyTotalRow(Long no) throws Exception{
		...
		try {
			...
			// 3.4.
			// 쿼리 확인
			System.out.println("BoardDAO.getReplyTotalRow().DBSQL.BOARD_GET_REPLY_TOTALROW : "
					+ DBSQL.BOARD_GET_REPLY_TOTALROW );
			...
			// TODO: handle exception
			e.printStackTrace();
			throw new Exception("게시판 댓글 데이터 전체 갯수를 가져오는 DB 처리 중 오류");
		} finally {
			DBInfo.close(con, pstmt, rs);
		}
	...
	public int replyWrite(BoardReplyVO vo) throws Exception {
		...
		try {
			...
			result = pstmt.executeUpdate();
			// 6. 표시
			System.out.println("게시판 댓글등록 성공");
			
		} catch (Exception e) {
			// TODO: handle exception
			e.printStackTrace();
			throw new Exception("BoardDAO - 게시판 댓글등록 처리 중 DB 오류가 발생됨.");
		} finally {
			DBInfo.close(con, pstmt);
		}
