...
public class ImageDAO {
	...
	public List<ImageVO> list(PageObject pageObject) throws Exception{
		...
		try {
			...
			e.printStackTrace();
			// 사용자를 위한 오류 처리
			throw new Exception("이미지 게시판 리스트 실행 중 DB 처리 오류");
		} finally {
			// 7. 닫기
	...
	public long getTotalRow() throws Exception{
		...
		try {
			...
			// 3.4.
			// 쿼리 확인
			System.out.println("ImageDAO.getTotalRo().DBSQL.IMAGE_GET_TOTALROW : "
			...
			// TODO: handle exception
			e.printStackTrace();
			throw new Exception("이미지 게시판 데이터 전체 갯수를 가져오는 DB 처리 중 오류");
		} finally {
			DBInfo.close(con, pstmt, rs);
		}
	...
	public ImageVO view(Long no) throws Exception{
		...
		try {
			...
			if(rs != null && rs.next()) {
				...
				vo.setWriteDate(rs.getString("writeDate"));
				vo.setFileName(rs.getString("fileName"));
				System.out.println("ImageDAO.view().result : " + vo);
			}
		} catch (Exception e) {
			// TODO: handle exception
			e.printStackTrace();
	...
	public int write(ImageVO vo) throws Exception {
		...
		try {
			...
			// TODO: handle exception
			e.printStackTrace();
			throw new Exception("이미지 등록 DB 처리중 오류");
		} finally {
	...
	public int updateFile(ImageVO vo) throws Exception {
		...
		try {
			...
			// TODO: handle exception
			e.printStackTrace();
			throw new Exception("이미지 파일 수정 DB 처리중 오류");
		} finally {
			DBInfo.close(con, pstmt);
		}
	...
	public int delete(Long no) throws Exception {
		...
		try {
			...
			// 6.
			System.out.println("ImageDAO.delete() - 이미지게시판 글 삭제 완료");
			
		} catch (Exception e) {
			// TODO: handle exception
			e.printStackTrace();
			throw new Exception("이미지 게시판 글 삭제 DB 처리중 오류");
		} finally {
			DBInfo.close(con, pstmt);
		}
