...
<script type="text/javascript">
	...
	$(function() { // jquery에서 익명함수를 전달해서 저장해놨다가 Document가 로딩이 다되면 호출해서 처리해준다.
		...
		$(".replyUpdateBtn").click(function() {
			...
			// alert(dataRow);
			// js : parseInt(str) -> str 안에 처음 만나는 숫자를 정수형숫자를 숫자로 꺼낸다.
		...
		$("#modal_writeReplyBtn").click(
				function() {
					// alert("등록 전송");
					// 데이터 유효성 검사. -> 생략
					// 데이터를 받는 URL을 정한다.
					$("#replyForm").attr("action",
							"replyWrite.do?page=1&perPageNum=10&no=${vo.no}");
	...
	<div class="container">
		...
		<table class="table">
			...
			<tfoot>
				<tr>
					<td colspan="2"><a
						href="updateForm.do?no=${vo.no }&page=${pageObject.page }&perPageNum=${pageObject.perPageNum}"
						class="btn btn-default">글수정</a> <a
						href="delete.do?no=${vo.no }&perPageNum=${pageObject.perPageNum}"
						class="btn btn-default" id="deleteBtn">글삭제</a> <a
						href="list.do?page=${pageObject.page }&perPageNum=${pageObject.perPageNum}"
		...
		<h3>
			...
			<button class="pull-right btn btn-default" data-toggle="modal"
				data-target="#myModal" id="writeReplyBtn">댓글달기</button>
		</h3>
		<ul class="list-group">
			...
			<c:forEach items="${list }" var="vo">
				...
				<pre style="border: none; padding: 0px; background: none;"><span class="rno">${vo.rno }. </span><span class="content">${vo.content }</span></pre> <span class="writer">${vo.writer }</span> - <span class="writeDate">${vo.writeDate }</span><span class="pull-right">
				...
				<div class="modal-body">
					<form action="replyWrite.do" method="post" id="replyForm">
						...
						<div class='form-group' id="modal_rno_div">
							<label for="rno">댓글번호</label> <input name="rno"
								readonly="readonly" id="modal_rno" class="form-control">
						</div>
						<div class="form-group">
							<label for="content">내용</label>
							<textarea name="content" rows="3" class="form-control"
								id="modal_content"></textarea>
						</div>
						<div class='form-group'>
							<label for="writer">작성자</label> <input name="writer"
								id="modal_writer" class="form-control">
						</div>
					</form>
				</div>
