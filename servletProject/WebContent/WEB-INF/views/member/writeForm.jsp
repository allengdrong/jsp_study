	...
	$(function() { // jquery에서 익명함수를 전달해서 저장해놨다가 Document가 로딩이 다되면 호출해서 처리해준다.
		...
		$("#writeForm").submit(function() {
			...
			if (!require($("#pw"), "비밀번호"))
				return false;
			// 비밀번호 확인
			if (!require($("#pw2"), "비밀번호 확인"))
				return false;

			// 길이
			// 제목 4자 이상
			if (!checkLength($("#title"), "제목", 4))
				return false;
			// 내용 4자 이상
			if (!checkLength($("#content"), "내용", 4))
				return false;
			// 작성자 2자 이상
		...
		// 아이디 중복 체크 하는 이벤트 처리
		$("#id").keyup(function() {
			// 결과 디자인 - warning : 잘 안됨. success : 잘 됨.
			$("#checkId").removeClass("alert-warning alert-success")
			// alert("아이디 중복 체크");
			var id = $("#id").val();
			// 아이디가 입력이 안되어 있거나 길이가 3 미만인 경우 처리
			if (!id || id.length < 3) {
			...
			// callback - load처리가 다 끝나고 호출되는 함수
			function(result) {
				// 넣은 글자가 "가능한" 포함하고 있으면 CSS를 성공으로 바꾼다.
				// alert(result);
				if ($("#checkId").text().indexOf("가능한") >= 0)
				// alert($("#checkId").text().indexOf("가능한") >= 0);
	...
	<div class="container">
		...
		<form action="write.do" id="writeForm" method="post">
			<div class="form-group">
				...
				<div id="checkId" class="alert alert-warning">아이디는 3자 이상이여야
					합니다.</div>
			</div>
			<div class="form-group">
				<label for="pw">비밀 번호</label> <input name="pw" id="pw"
					class="form-control" required="required" type="password"
					placeholder="비밀번호 입력 - 4자 이상" maxlength="20" />
			</div>
			<div class="form-group">
				<label for="pw">비밀 번호 확인</label> <input name="pw2" id="pw2"
					class="form-control" required="required" type="password"
					placeholder="비밀번호 확인 입력 - 4자 이상" maxlength="20" />
			</div>
			<div id="checkPw" class="alert alert-warning">비밀번호와 비밀번호 확인은 4자
				이상이여야 합니다.</div>
			<div class="form-group">
			...
			<div class="form-group">
				<h4>성별</h4>
				<label for="gender_man" class="radio-inline"> <input
					name="gender" id="gender_man" type="radio" checked="checked"
					value="남자" /> 남자
				</label> <label for="gender_woman" class="radio-inline"> <input
					name="gender" id="gender_woman" type="radio" value="여자" /> 여자
				</label>
			</div>
			<div class="form-group">
				<label for="birth">생년월일</label> <input name="birth" id="birth"
					class="datepicker form-control" />
			</div>
			<div class="form-group">
				<label for="tel">연락처</label>
				<div class="form-inline">
					<input name="tel" id="tel1" class="form-control" size="3"
						maxlength="3" /> - <input name="tel" id="tel2"
						class="form-control" size="4" maxlength="4" /> - <input name="tel"
						id="tel3" class="form-control" size="4" maxlength="4" />
				</div>
			</div>
			<div class="form-group">
