...
public class MemberDAO {
	...
	public LoginVO login(LoginVO vo) throws Exception {
		...
		try {
			...
			e.printStackTrace();
			throw new Exception("로그인 DB 처리중 오류");
		} finally {
	...
	public int gradeModify(MemberVO vo) throws Exception {
		...
		try {
			...
			// result가 1이면 수정이 한개 수정이 일어난다. result가 0이면 수정이 되지 않았다. (조건에 맞는 데이터가 없다)
			...
			// TODO: handle exception
			e.printStackTrace();
			throw new Exception("회원등급 변경 DB 처리중 오류");
		} finally {
	...
	public MemberVO view(String id) throws Exception{
		// 결과를 저장해서 넘겨 줘야할 변수. 데이터가 없어서 초기값 null을 셋팅. 숫자면 0을 셋팅
		MemberVO vo = null;
