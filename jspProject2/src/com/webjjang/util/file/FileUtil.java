...
public class FileUtil {
	...
	public static boolean remove(String fileName) throws Exception {
		// 1. 문자열을 파일 객체로 만든다. 2. 있는지 확인한다. 3. 삭제한다. 4. 결과 return
		File file = toFile(fileName);
		// 파일이 존재하지 않는 경우 예외 발생
		if(!exist(file)) throw new Exception("FileUtil.remove() - 삭제하려는 파일이 존재하지 않습니다.");
		// 파일이 존재하는 경우의 처리
		if(!delete(file)) throw new Exception("FileUtil.remove() - 삭제하려는 파일이 삭제되지 않습니다.");
		System.out.println("FileUtil.remove() - 파일이 삭제되었습니다.");
		return true;
