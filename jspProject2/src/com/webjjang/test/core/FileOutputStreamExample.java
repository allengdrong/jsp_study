...
// java에서 파일을 읽어 들여 와서 byte 단위의 데이터들을 차례대로 지정된 파일에 저장하는 프로그램
public class FileOutputStreamExample {
	...
	public static void main(String[] args) throws Exception {
		...
		String originalFileName = "c:/Temp/icon.gif"; // 이미지 파일명 작성 - 파일이 존재해야만 한다.
		// 저장할 파일의 정보 - 프로그램을 실행하기 전에는 존재하지 않는다.
		// 저장을 하려면 폴더는 반드시 존재해야만 한다. -> 폴더를 만든다.
		...
		// 파일 읽기 객체와 저장 객체를 생성해준다.
		FileInputStream fis = new FileInputStream(originalFileName);
		FileOutputStream fos = new FileOutputStream(targetFileName);
		
		int readByteNo = 0; // byte(char) -> 숫자로 운영 : int에 넣을 수 있다. 
		// 아래 readBytes에 읽어온 데이터의 갯수를 저장하는 변수
		...
		// 문자열인 경우 모두 + 숫자이거나 0 이 나와야 한다. 읽었을때 -1이면 모두 읽어서 읽을 것이 더 이상 없다.
